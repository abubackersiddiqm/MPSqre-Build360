...
class Command(BaseCommand):
    help = "Grant Phase 43 land acquisition permissions to a tenant role."
    ...
    def handle(self, *args, **options):
        ...
        permissions = Permission.objects.filter(code__startswith="land.")
        created = 0
        for permission in permissions:
            _, was_created = RolePermission.objects.get_or_create(role=role, permission=permission)
            created += int(was_created)
        self.stdout.write(self.style.SUCCESS(f"Reconciled {permissions.count()} land permissions for {role.name}; created {created} grants."))
