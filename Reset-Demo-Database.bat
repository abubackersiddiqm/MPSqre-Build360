...
set /p CONFIRM=Type RESET-DEMO to erase ONLY the demo database data: 
if /I not "%CONFIRM%"=="RESET-DEMO" (echo Cancelled.& exit /b 2)
cd /d "%ROOT%\backend"
...
"%PYTHON%" manage.py flush --noinput || exit /b 1
"%PYTHON%" manage.py migrate || exit /b 1
"%PYTHON%" manage.py seed_build360_demo || exit /b 1
echo [SUCCESS] Demo database was reset and reseeded.
