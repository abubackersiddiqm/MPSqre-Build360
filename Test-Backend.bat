@echo off
setlocal
set "ROOT=%~1"
if not defined ROOT set "ROOT=%~dp0"
for %%I in ("%ROOT%") do set "ROOT=%%~fI"

set "PYTHON=%ROOT%\backend\.venv\Scripts\python.exe"
if not exist "%PYTHON%" (
  echo [ERROR] backend\.venv runtime missing. Run Setup-Backend-Runtime.bat first.
  exit /b 2
)
if not exist "%ROOT%\backend\.env.testing" (
  echo [ERROR] backend\.env.testing missing.
  exit /b 3
)

cd /d "%ROOT%\backend"
set "BUILD360_ENVIRONMENT=testing"
set "APP_ENV=test"
set "APP_VERSION=1.0.0"
set "DJANGO_ENV_FILE=backend\.env.testing"

echo [INFO] Build360 backend pytest gate.
echo [INFO] Tests reuse the dedicated build360_testing database.
echo [INFO] The application DB role remains least-privileged; CREATEDB is not required.

"%PYTHON%" manage.py build360_environment_status --require testing || exit /b 1
"%PYTHON%" -m pytest --reuse-db
exit /b %ERRORLEVEL%
