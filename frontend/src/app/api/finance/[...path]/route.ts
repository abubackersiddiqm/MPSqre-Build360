...
async function proxy(request: NextRequest, context: RouteContext) {
  ...
  if (!["GET", "HEAD"].includes(method) && !assertSameOrigin(request)) {
    return NextResponse.json({ code: "FINANCE-ORIGIN-REJECTED", message: "Invalid request origin." }, { status: 403 });
  }
  const [token, companyPublicId, params] = await Promise.all([accessToken(), selectedCompany(), context.params]);
  if (!token || !companyPublicId) {
    return NextResponse.json({ code: "FINANCE-AUTH-REQUIRED", message: "Authentication and company selection are required." }, { status: 401 });
  }
  const path = `/finance/${params.path.map(encodeURIComponent).join("/")}${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error("Build360 Finance backend is unavailable", { error: error instanceof Error ? error.message : "Unknown backend connection error" });
    return NextResponse.json({ code: "FINANCE-BACKEND-UNAVAILABLE", message: "Finance service is unavailable. Confirm the Django API is running." }, { status: 503 });
  }
}
