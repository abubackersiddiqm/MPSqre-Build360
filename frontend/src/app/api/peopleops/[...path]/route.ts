...
async function proxy(request: NextRequest, context: RouteContext) {
  ...
  if (!["GET", "HEAD"].includes(method) && !assertSameOrigin(request)) {
    return NextResponse.json({ code: "PEOPLEOPS-ORIGIN-REJECTED", message: "Invalid request origin." }, { status: 403 });
  }
  const [token, companyPublicId, params] = await Promise.all([accessToken(), selectedCompany(), context.params]);
  if (!token || !companyPublicId) {
    return NextResponse.json({ code: "PEOPLEOPS-AUTH-REQUIRED", message: "Authentication and company selection are required." }, { status: 401 });
  }
  const path = `/people/${params.path.map(encodeURIComponent).join("/")}${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error("Build360 People Operations backend is unavailable", { error: error instanceof Error ? error.message : "Unknown backend connection error" });
    return NextResponse.json({ code: "PEOPLEOPS-BACKEND-UNAVAILABLE", message: "People Operations is unavailable. Confirm the Django API is running." }, { status: 503 });
  }
}
