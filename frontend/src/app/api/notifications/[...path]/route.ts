...
async function proxy(request: NextRequest, context: RouteContext) {
  ...
  if (!["GET", "HEAD"].includes(method) && !assertSameOrigin(request)) {
    return NextResponse.json(
      { code: "NOTIFICATION-ORIGIN-REJECTED", message: "Invalid request origin." },
      { status: 403 },
    );
  }
  ...
  if (!token || !companyPublicId) {
    return NextResponse.json(
      {
        code: "NOTIFICATION-AUTH-REQUIRED",
        message: "Authentication and company selection are required.",
      },
      { status: 401 },
    );
  }
  const path = `/notifications/${params.path.map(encodeURIComponent).join("/")}${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error("Build360 Notification backend is unavailable", {
      error: error instanceof Error ? error.message : "Unknown backend connection error",
    });
    return NextResponse.json(
      {
        code: "NOTIFICATION-BACKEND-UNAVAILABLE",
        message: "Notification service is unavailable. Confirm the Django API is running.",
      },
      { status: 503 },
    );
  }
