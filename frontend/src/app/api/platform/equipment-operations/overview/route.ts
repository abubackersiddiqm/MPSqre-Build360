...
export async function GET() {
  ...
  try {
    const backendResponse = await fetch(
      backendUrl("/equipment-operations/overview/"),
      {
        headers: {
    ...
    const body: unknown = await backendResponse.json().catch(() => ({
      message: "Equipment operations could not be loaded.",
    }));

    return NextResponse.json(body, { status: backendResponse.status });
  } catch {
    return NextResponse.json(
      { message: "The equipment operations API is temporarily unavailable." },
      { status: 502 },
    );
  }
}
