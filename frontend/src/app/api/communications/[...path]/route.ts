...
async function proxy(request: NextRequest, context: RouteContext) {
  ...
  if (!["GET", "HEAD"].includes(method) && !assertSameOrigin(request)) {
    return NextResponse.json(
      { code: "COMMUNICATION-ORIGIN-REJECTED", message: "Invalid request origin." },
      { status: 403 },
    );
  }
  ...
  if (!token || !companyPublicId) {
    return NextResponse.json(
      {
        code: "COMMUNICATION-AUTH-REQUIRED",
        message: "Authentication and company selection are required.",
      },
      { status: 401 },
    );
  }
  const path = `/communications/${params.path.map(encodeURIComponent).join("/")}${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error("Build360 Communication backend is unavailable", {
      error: error instanceof Error ? error.message : "Unknown backend connection error",
    });
    return NextResponse.json(
      {
        code: "COMMUNICATION-BACKEND-UNAVAILABLE",
        message: "Communication service is unavailable. Confirm the Django API is running.",
      },
      { status: 503 },
    );
  }
